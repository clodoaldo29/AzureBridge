...
interface WorkItemsByTypeChartProps {
    workItems: WorkItem[];
}
...
export function WorkItemsByTypeChart({ workItems }: WorkItemsByTypeChartProps) {
    const { data, total } = useMemo(() => {
        ...
        const data: SliceData[] = Array.from(counts.entries())
            .map(([name, value]) => ({
                name,
                value,
                color: TYPE_COLORS[name] || DEFAULT_COLOR,
                _total: total,
            }))
    ...
    if (total === 0) {
        return (
            <Card>
                <CardHeader><CardTitle className="text-base">Work Items por Tipo</CardTitle></CardHeader>
                <CardContent>
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle className="text-base">Work Items por Tipo</CardTitle>
            </CardHeader>
            <CardContent>
